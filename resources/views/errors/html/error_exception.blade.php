...
<body onload="init()">
    ...
    <div class="header">
        ...
        <div class="container">
            <h1>{{ $title, $exception->getCode( ? ' #' . $exception->getCode() : '') }}</h1>
            <p>
    ...
    <pre>
    Caused by:
    {{ $prevException::class, $prevException->getCode( ? ' #' . $prevException->getCode() : '') }}

    {{ nl2br($prevException->getMessage()) }}
                        ...
                        <p>
                            ...
                            <?php if (isset($row['class'])) : ?>
                                ...
                                <?php if (! empty($row['args'])) : ?>
                                    ...
                                    ( <a href="#" onclick="return toggle('{{ $argsId, 'attr' }}');">arguments</a> )
                                    <div class="args" id="{{ $argsId, 'attr' }}">
                                        <table cellspacing="0">
                                        ...
                                        foreach ($row['args'] as $key => $value) : ?>
                                            <tr>
                                                <td><code>{{ isset($params[$key] ? '$' . $params[$key]->name : "#{$key}") }}</code></td>
                        ...
                        <?php foreach ($headers as $name => $value) : ?>
                            <tr>
                                ...
                                if ($value instanceof Header) {
                                    echo $value->getValueLine(, 'html');
                                } else {
                                    foreach ($value as $i => $header) {
                                        echo ' ('. $i+1 . ') ' . $header->getValueLine(, 'html');
                                    }
                                }
                                ?>
                                </td>
                    ...
                    <table>
                        ...
                        <?php foreach ($headers as $name => $value) : ?>
                            <tr>
                                ...
                                <?php
                                if ($value instanceof Header) {
                                    echo $response->getHeaderLine($name, 'html');
                                } else {
                                    foreach ($value as $i => $header) {
                                        echo ' ('. $i+1 . ') ' . $header->getValueLine(, 'html');
                                    }
